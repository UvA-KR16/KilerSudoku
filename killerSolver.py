...
for i in range (0, 9):
    for j in range (0, 9):
        for num in range (1, 10): # here, it represent num of 1 to 9
            indexBoard[i][j][num-1] = index(i, j, num)
...
def encode_to_cnf(killerRules): #encode a problem (stored in matrix) as cnf
    ...
    for k in killerRules:
        ...
        for num in common:
            ...
            for cc in cageCells:
                cnf.append([-1* indexBoard[cc[0]][cc[1]], numIndex]) # if the number appears in one of the cells, then it makes the 
        ...
        # for those cages in the same row, column, nonet, we can also restrict other cells.
